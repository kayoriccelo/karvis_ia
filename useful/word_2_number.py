...
def number_formation(number_words):
    ...
    for number_word in number_words:
        numbers.append(pt_br_number_system[number_word])
...
def get_decimal_sum(decimal_digit_words):
    ...
    for dec_word in decimal_digit_words:
        if dec_word not in decimal_words:
            ...
            decimal_number_str.append(pt_br_number_system[dec_word])
...
def word_to_num(number_sentence):
    if type(number_sentence) is not str:
        raise ValueError(
            "Type of input is not string! Please enter a valid number word (eg. \'two million twenty three thousand "
            "and forty nine\')"
        )
    ...
    for word in split_words:
        if word in pt_br_number_system:
            clean_numbers.append(word)

    # Error message if the user enters invalid input!
    if len(clean_numbers) == 0:
        raise ValueError(
            "No valid number words found! Please enter a valid number word (eg. two million twenty three thousand and "
            "forty nine)"
        )
    ...
    if clean_numbers.count('mil') > 1 or clean_numbers.count('milhão') > 1 or clean_numbers.count('bilhão') > 1 or clean_numbers.count('point') > 1:
        raise ValueError(
            "Redundant number word! Please enter a valid number word (eg. two million twenty three thousand and forty "
            "nine)"
        )
    ...
    if len(clean_numbers) > 0:
        ...
        if len(clean_numbers) == 1:
            total_sum += pt_br_number_system[clean_numbers[0]]
